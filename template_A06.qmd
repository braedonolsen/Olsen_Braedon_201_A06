...
[https://github.com/braedonolsen/Olsen_Braedon_201_A06](https://github.com/braedonolsen/Olsen_Braedon_201_A06)
...
for (player in names(player_info)){
  player_info[[player]]$height <- player_height[player]
}
...
```{r}
ppg_rounded <- sapply(points_per_game, round, 2)
...
```{r}
round_to_digits <- function(num){
  if(nchar(as.character(num)) == 2){
    round(num, digits = -1)
  } else if(nchar(as.character(num)) == 3){
    round(num, digits = -2)
  } else if(nchar(as.character(num)) == 4){
    round(num, digits = -3)
  }
}

mat_rand_rounded <- apply(mat_rand, MARGIN = 1:2, round_to_digits)
...
```{r}
max_temps_colorado <- max_temps[max_temps$state == 'CO', ]
max_temps_ohio <- max_temps[max_temps$state == 'OH', ]
max_temps_virginia <- max_temps[max_temps$state == 'VA', ]
```
...
```{r}
num_temps_colorado <- as.data.frame(apply(max_temps_colorado, 1:2, as.numeric))

num_temps_colorado <- num_temps_colorado[, -c(1, 2)]

median_temps <- apply(num_temps_colorado, 1, median)

years <- 1895:2020

medtemp_per_year <- data.frame(year = years, median_temp = median_temps)

max_med_temp <- medtemp_per_year$median_temp[which.max(medtemp_per_year$median_temp)]
year_max <- medtemp_per_year$year[which.max(medtemp_per_year$median_temp)]
min_med_temp <- medtemp_per_year$median_temp[which.min(medtemp_per_year$median_temp)]
year_min <- medtemp_per_year$year[which.min(medtemp_per_year$median_temp)]

print(paste0("The maximum median annual temperature of ", max_med_temp, " occurred in ", year_max, " and the minimum median annual temperature of ", min_med_temp, " occurred in ", year_min, "."))
